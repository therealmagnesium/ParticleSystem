...
Sandbox::Sandbox(const AppInfo& info) : Application(info)
{
    ...
    m_blueParticleState.dieSpeed = 500.f;
    ...
    m_orangeParticleState.spawnRate = 0.01f;
    m_orangeParticleState.dieSpeed = 500.f;
...
void Sandbox::Update()
{
    ...
    m_blueParticleState.velocity = sf::Vector2f((rand() % 800) - 400.f, (rand() % 200) - 600.f);
    m_blueParticleState.acceleration = sf::Vector2f(0.f, 150.f);
    
    m_orangeParticleState.velocity = sf::Vector2f((rand() % 600) - 300.f, (rand() % 200) - 600.f);
    m_orangeParticleState.acceleration = sf::Vector2f(-50.f, 150.f);
